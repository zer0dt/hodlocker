...
import { HODLTransactions, broadcastTx, getAddressUtxos, postNewReply } from '../../server-actions'
...
const privKey = process.env.ANON_PRIVATE_KEY as string

export const postAnonReply = async (postTxid: string, note: string, amountToLock: string, nLockTime: number): Promise<HODLTransactions | undefined> => {
  if (!privKey) {
      console.error("No private key to fund anon posting found.");
      return;
  }
  ...
  try {
      ...
      console.error("Error in postAnonReply function:", error);
      throw error
  }
};
