...
import { getBitcoinerLikedData } from "./get-bitcoiner-avatar-rank"
...
export default async function PostProfileImage({ avatar, handle }: PostProfileImageProps) {
  ...
  const bitcoiner = await getBitcoinerLikedData(handle)
    ...
    return (
        ...
        height={40}
        className={`rounded-full aspect-square ring-4 ${getRingColor(bitcoiner ? bitcoiner.totalLockLikedFromOthers : 0)}`}
      />
    )
}