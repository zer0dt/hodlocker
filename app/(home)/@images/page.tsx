...
const getImagePosts = (async (sort: string, filter: number, filter2: number, page: number, limit: number): Promise<[]> => {
...
const getCachedPosts = unstable_cache(
    ...
    ['image-posts'],
    {
        tags: ['images', 'posts'], // Cache tags for invalidation
        revalidate: 60, // Revalidation time in seconds (e.g., 1 hour)
    }
);

export default async function ImagesFeed({ searchParams }: ImagesFeedProps) {
    ...
    if (activeTab == "images") {
        const imagePosts = await getCachedPosts(activeSort, activeFilter, activeFilter2, currentPage, 30)

        const jsonPosts = JSON.stringify(imagePosts, null, 2);
        const sizeInBytes = new Blob([jsonPosts]).size;
        const sizeInMB = sizeInBytes / (1024 * 1024); // Convert bytes to MB
        console.log("Size of imagePosts:", sizeInMB.toFixed(2), "MB");

        const posts = parse<HODLTransactions[]>(imagePosts)
